...
async function initDatabase() {
  try {
    ...
    // 7. Reward Allocations table (DM Team → ASM top-level awards)
    await pool.query(`
    ...
    // 8. Reward Distributions table (ASM → FSM / SD / Shop cascades)
    await pool.query(`
    ...
    // 9. Reward Audit Logs table (immutable event trail)
    await pool.query(`
    ...
    // Expand reward_audit_logs event_type constraint to include 'customer_rewarded'
    try {
      await pool.query(`ALTER TABLE reward_audit_logs DROP CONSTRAINT IF EXISTS reward_audit_logs_event_type_check`);
      await pool.query(`
        ALTER TABLE reward_audit_logs ADD CONSTRAINT reward_audit_logs_event_type_check
        CHECK (event_type IN ('issued','distributed','acknowledged','viewed','customer_rewarded'))
      `);
    } catch (e) {
      console.log('reward_audit_logs constraint update skipped:', e.message);
    }

    // 10. Customer Rewards table (DM Team → end consumer loyalty rewards)
    await pool.query(`
